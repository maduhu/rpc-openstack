...
Deployments of features/projects are handled via deployment variables in the [deploy.sh script](https://github.com/rcbops/rpc-openstack/blob/master/scripts/deploy.sh). Too add the ability for a feature/project to be deployed, add the proper DEPLOY variable and use that as a trigger to execute the instructions necessary to deploy said feature/project.
...
TODO: (alextricity25) Add references to the deploy.sh script documentation.
